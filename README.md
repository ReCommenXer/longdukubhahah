function loadcheck()
    if isfile("RebornXer Hub Fisch "..game.Players.LocalPlayer.Name..".json") then
    else
    writefile("RebornXer Hub Fisch "..game.Players.LocalPlayer.Name..".json",game:GetService("HttpService"):JSONEncode(_G.SST))
    return
    end
    end
    pcall(function()
    ...
    function LoadSetting()
        if isfile("RebornXer Hub Fisch "..game.Players.LocalPlayer.Name..".json") then
            -- โหลดไฟล์
            local fileContent = readfile("RebornXer Hub Fisch "..game.Players.LocalPlayer.Name..".json")
...
Main:AddDropdownRight("Select Mode",Mode_List,_G.SST.Select_Mode,function(a)
    Select_Mode = a
    _G.SST.Select_Mode = Select_Mode
	SaveSetting()
	if _G.SST.Select_Mode == "Fast" then
		Reeldeley = 0.6
		Shackdeley = 0.2
	else
		Reeldeley = 1
...
function Shake()
	isReeling = true -- เปิดการทำงาน

	PlayerGUI.ChildAdded:Connect(function(GUI)
		if not isReeling then return end -- หยุดทันทีถ้าถูกปิด
		if GUI:IsA("ScreenGui") and GUI.Name == "shakeui" then
			if GUI:FindFirstChild("safezone") ~= nil then
				GUI.safezone.ChildAdded:Connect(function(child)
					if not isReeling then return end -- หยุดทันทีถ้าถูกปิด
					if child:IsA("ImageButton") and child.Name == "button" then
							task.wait(Shackdeley)
							if child.Visible == true then
								while WaitForSomeone(RenderStepped) do
									if not isReeling then break end -- ออกจากลูปถ้าถูกปิด
									if GUI.safezone:FindFirstChild(child.Name) ~= nil then
										task.wait()
										pcall(function()
...
function StopReel()
	isReeling = false -- ปิดการทำงาน
	GuiService.SelectedObject = nil -- รีเซ็ตตัวเลือก GUI
end
...
function EquipRod()
    ...
    function Cast()
		wait(.1)
		game:GetService("Players").LocalPlayer.Character:FindFirstChild(chackrod).events.cast:FireServer(100)
    end
